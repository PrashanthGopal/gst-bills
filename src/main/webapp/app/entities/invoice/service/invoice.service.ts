...
type RestOf<T extends IInvoice | NewInvoice> = Omit<T, 'createDateTime' | 'updateDateTime'> & {
...
export class InvoiceService {
  ...
  protected convertDateFromClient<T extends IInvoice | NewInvoice | PartialUpdateInvoice>(invoice: T): RestOf<T> {
    return {
      ...invoice,
      createDateTime: invoice.createDateTime?.toJSON() ?? null,
      updateDateTime: invoice.updateDateTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restInvoice: RestInvoice): IInvoice {
    return {
      ...restInvoice,
      createDateTime: restInvoice.createDateTime ? dayjs(restInvoice.createDateTime) : undefined,
      updateDateTime: restInvoice.updateDateTime ? dayjs(restInvoice.updateDateTime) : undefined,
    };
  }

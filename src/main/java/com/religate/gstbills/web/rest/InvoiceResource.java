...
/**
 * REST controller for managing {@link com.religate.gstbills.domain.Invoice}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Invoice> result = invoiceRepository
            ...
            .map(existingInvoice -> {
                ...
                if (invoice.getCreateDateTime() != null) {
                    existingInvoice.setCreateDateTime(invoice.getCreateDateTime());
                }
                if (invoice.getUpdateDateTime() != null) {
                    existingInvoice.setUpdateDateTime(invoice.getUpdateDateTime());
                }
                ...
                if (invoice.getTotalIgstRate() != null) {
                    existingInvoice.setTotalIgstRate(invoice.getTotalIgstRate());
                }
                if (invoice.getTotalCgstRate() != null) {
                    existingInvoice.setTotalCgstRate(invoice.getTotalCgstRate());
                }
                if (invoice.getTotalSgstRate() != null) {
                    existingInvoice.setTotalSgstRate(invoice.getTotalSgstRate());
                }
                if (invoice.getTotalTaxRate() != null) {
                    existingInvoice.setTotalTaxRate(invoice.getTotalTaxRate());
                }
                if (invoice.getTotalAssAmount() != null) {
                    existingInvoice.setTotalAssAmount(invoice.getTotalAssAmount());
                }
                if (invoice.getTotalNoItems() != null) {
                    existingInvoice.setTotalNoItems(invoice.getTotalNoItems());
                }
                if (invoice.getGrandTotal() != null) {
                    existingInvoice.setGrandTotal(invoice.getGrandTotal());
                }
                if (invoice.getDistance() != null) {
                    existingInvoice.setDistance(invoice.getDistance());
                }
                if (invoice.getModeOfTransaction() != null) {
                    existingInvoice.setModeOfTransaction(invoice.getModeOfTransaction());
                }
                if (invoice.getTransType() != null) {
                    existingInvoice.setTransType(invoice.getTransType());
                }
                if (invoice.getVehicleNo() != null) {
                    existingInvoice.setVehicleNo(invoice.getVehicleNo());
                }

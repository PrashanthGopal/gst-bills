...
/**
 * REST controller for managing {@link com.religate.gstbills.domain.Orders}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Orders> result = ordersRepository
            ...
            .map(existingOrders -> {
                ...
                if (orders.getTitle() != null) {
                    existingOrders.setTitle(orders.getTitle());
                }
                if (orders.getDescription() != null) {
                    existingOrders.setDescription(orders.getDescription());
                }
                if (orders.getStatus() != null) {
                    existingOrders.setStatus(orders.getStatus());
                }
                if (orders.getOrdersItemsId() != null) {
                    existingOrders.setOrdersItemsId(orders.getOrdersItemsId());
                }

...
import { EntityArrayResponseType, ProductUnitService } from '../service/product-unit.service';
...
export class ProductUnitComponent implements OnInit {
  ...
  trackId = (_index: number, item: IProductUnit): number => this.productUnitService.getProductUnitIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.productUnits = this.refineData(dataFromBody);
  }

  protected refineData(data: IProductUnit[]): IProductUnit[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IProductUnit[] | null): IProductUnit[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.productUnitService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }

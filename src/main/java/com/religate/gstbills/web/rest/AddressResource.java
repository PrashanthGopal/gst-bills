...
import com.religate.gstbills.domain.Address;
import com.religate.gstbills.repository.AddressRepository;
...
/**
 * REST controller for managing {@link com.religate.gstbills.domain.Address}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public List<Address> getAllAddresses(@RequestParam(required = false) String filter) {
        /*if ("organization-is-null".equals(filter)) {
            ...
            return StreamSupport
                ...
                .filter(address -> address.getOrganization() == null)
                .toList();
        }

        if ("clients-is-null".equals(filter)) {
            log.debug("REST request to get all Addresss where clients is null");
            return StreamSupport
                .stream(addressRepository.findAll().spliterator(), false)
                .filter(address -> address.getClients() == null)
                .toList();
        }

        if ("transporter-is-null".equals(filter)) {
            log.debug("REST request to get all Addresss where transporter is null");
            return StreamSupport
                .stream(addressRepository.findAll().spliterator(), false)
                .filter(address -> address.getTransporter() == null)
                .toList();
        }

        if ("invoice-is-null".equals(filter)) {
            log.debug("REST request to get all Addresss where invoice is null");
            return StreamSupport
                .stream(addressRepository.findAll().spliterator(), false)
